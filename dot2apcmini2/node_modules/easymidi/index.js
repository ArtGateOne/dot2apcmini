...
class Input extends EventEmitter {
  constructor(name, virtual) {
    ...
    this.portNumberedNames = getInputs();
    ...
    if (virtual) {
      ...
      for (let i = 0; i < numInputs; i++) {
        if (name === this.portNumberedNames[i]) {
          found = true;
...
class Output {
  constructor(name, virtual) {
    ...
    this.portNumberedNames = getOutputs();
...
const getInputs = () => {
  ...
  for (let i = 0; i < input.getPortCount(); i++) {
    ...
    var numberPortName = portName;
    while(inputs.includes(numberPortName)) {
      counter++;
      numberPortName = portName + counter;
    }
    inputs.push(numberPortName);
  }
  input.closePort();
  return inputs;
}
...
const getOutputs = () => {
  ...
  for (let i = 0; i < output.getPortCount(); i++) {
    ...
    var numberPortName = portName;
    while(outputs.includes(numberPortName)) {
      counter++;
      numberPortName = portName + counter;
    }
    outputs.push(numberPortName);
  }
  output.closePort();
  return outputs;
}
